<project name="jacoco-core.0.6.5" xmlns:jacoco="antlib:org.jacoco.ant" default="test" basedir=".">
	...
	<property name="java.classes" location="${build}/org.jacoco.core/classes" />
	<property name="test.src" location="org.jacoco.core.test/src" />
	<property name="test.classes" location="${build}/org.jacoco.core.test/test-classes" />
...
<!--===================== Jacoco task ================================== -->
 ...
 <!-- ========== Unit Tests ========================================= -->
    ...
    <jacoco:coverage destfile="${build}/jacocoReport/jacoco.exec">
        <junit printsummary="true" showoutput="false" fork="yes" forkmode="once" haltonerror="true">
        <!--<formatter type="plain" usefile="false" />-->
        <formatter type="brief" usefile="false"/>
	<classpath>
                <path  refid="build.libs"/>
		<pathelement location="${test.src}/org/jacoco/core/test/validation/targets/Target02.java" />
		<pathelement location="${java.classes}" />
		<pathelement location="${test.classes}" />
        </classpath>
            <batchtest fork="true"> <!--todir="${build}/test-reports"-->
                <fileset dir="${test.src}">
		    <exclude name="org/jacoco/core/runtime/RuntimeTestBase.java" />
		    <exclude name="org/jacoco/core/runtime/TestStorage.java" />
                    <include name="**/*Test.java"/>
                    <include name="**/*Test*.java"/>
                    <!--<exclude name="**/InstrumenterTest.java" />-->
                </fileset>
            </batchtest>
        </junit>
   </jacoco:coverage>
...
<target name="jacoco.report" depends="test">
 ...
 <!-- ========== Mutation test ========================================= -->
    <target name="mutation-test" description="Run mutation analysis on all unit test using MAJOR">
        <junit printsummary="false" showoutput="false" haltonerror="false" mutationAnalysis="true" resultFile="results.csv" killDetailsFile="killed.csv">
        <classpath>
		<path  refid="build.libs"/>
                <pathelement location="${java.classes}"/>
                <pathelement location="${test.classes}"/>
            ...
            <batchtest>
                <fileset dir="${test.src}">
		    <exclude name="org/jacoco/core/instr/InstrumenterTest.java" />
		    <exclude name="org/jacoco/core/internal/analysis/ClassAnalyzerTest.java" />
                    <include name="**/*Test.java"/>
                    <include name="**/*Test*.java"/>
                </fileset>
            </batchtest>
