...
<!--
        ...
        $Id: build.xml 1297745 2012-03-06 21:56:02Z ggregory $
-->
        ...
        <fileset dir="${test.home}">
          ...
          <exclude name="**/*AbstractTestCase*"/>
            <exclude name="**/Abstract*TestCase*"/>
         	<!-- exclude the tests that cause failures or errors -->
    	        	<exclude name="**/FileUtilsTestCase.java"/>
    	        	<exclude name="**/BOMInputStreamTest.java"/>
    	        	<exclude name="**/ReversedLinesFileReaderTestParamBlockSize.java"/>
    	        	<exclude name="**/ReversedLinesFileReaderTestParamFile.java"/>
    	        	<exclude name="**/ReversedLinesFileReaderTestSimple.java"/>
        </fileset>
      </batchtest>
    </junit>
  </target>
	
  <target name="mutation-test" depends="compile.tests" description="Run mutation analysis of tests">
		<mkdir dir="${build.home}/ant-reports"/>
		<junit printsummary="false" showoutput="false" haltonfailure="${test.failonerror}" mutationAnalysis="true" resultFile="results.csv" killDetailsFile="killed.csv">
		      <classpath refid="test.classpath"/>
		      <!-- If test.entry is defined, run a single test, otherwise run all valid tests -->
		      <test name="${test.entry}" if="test.entry"/>
		      <batchtest todir="${build.home}/ant-reports" unless="test.entry">
